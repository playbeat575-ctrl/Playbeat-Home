 ...
 * - Renders a real product image (coverImage) with a subtle gradient overlay
 *   for brand consistency and text legibility.
 * - Falls back to the gradient + lucide-icon treatment when no image is set.
 */
    ...
    <div className={cn('relative overflow-hidden bg-secondary', className)}>
      {hasImage ? (
        <>
          ...
          />
          {/* gradient wash for brand cohesion + legibility */}
          <div className={cn('absolute inset-0 bg-gradient-to-br opacity-55 mix-blend-multiply', gradient)} />
          <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/10 to-transparent" />
          {showShine && (
            <div className="pointer-events-none absolute -inset-x-10 -top-12 h-32 rotate-12 bg-white/20 blur-2xl" />
          )}
        </>
      ) : (
        <>
